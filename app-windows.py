...
app = Flask(__name__)
app.config['CA_DIR'] = '_internal/ca'
app.config['CERTS_DIR'] = '_internal/certs'
app.config['UPLOAD_DIR'] = '_internal/uploads'
...
def download(cert_dir, filename):
    ...
    if cert_dir == 'ca':
        # 如果是CA证书目录
        full_path = os.path.join(base_dir, 'ca', filename)
    else:
        # 如果是普通证书目录
        full_path = os.path.join(base_dir, 'certs', cert_dir, filename)
...
def init_proxy_data_file():
    ...
    if not proxy_data_file:
        app.config['PROXY_DATA_FILE'] = os.path.join(os.path.dirname(__file__), 'proxy', 'proxy_data.json')
        proxy_data_file = app.config['PROXY_DATA_FILE']
    
    if not os.path.exists(proxy_data_file):
        os.makedirs(os.path.dirname(proxy_data_file), exist_ok=True)
...
def get_proxy_pid(service_id):
    try:
        print(f"[日志] 开始获取服务 {service_id} 的进程ID")
        # 读取proxy_data.json文件获取服务信息
        proxy_data_file = app.config.get('PROXY_DATA_FILE')
        if not os.path.exists(proxy_data_file):
            print(f"[日志] 代理数据文件 {proxy_data_file} 不存在")
            return jsonify({'success': True, 'pid': None})
            
        print(f"[日志] 正在读取代理数据文件 {proxy_data_file}")
        with open(proxy_data_file, 'r', encoding='utf-8') as f:
        ...
        if not service:
            print(f"[日志] 未找到服务ID为 {service_id} 的配置信息")
            return jsonify({'success': True, 'pid': None})
        ...
        result = subprocess.run(cmd, capture_output=True, text=True, shell=True)
        if result.stderr:
            print(f"[日志] 命令执行出错: {result.stderr}")
            return jsonify({'success': True, 'pid': None})
        
        # 解析输出获取进程ID
        output_lines = result.stdout.strip().split('\n')
        
        print(f"[日志] 命令输出内容: {output_lines}")
        ...
        if len(output_lines) > 0:
            ...
            if last_line.isdigit():
                pid = int(last_line)
                print(f"[日志] 成功获取进程ID: {pid}")
                return jsonify({'success': True, 'pid': pid})
            else:
                print(f"[日志] 最后一行不是有效的进程ID")
        else:
            print(f"[日志] 命令输出为空，未找到进程")
        
        print(f"[日志] 未找到服务 {service_id} 的进程ID")
        return jsonify({'success': True, 'pid': None})
        
    except Exception as e:
        print(f"[日志] 获取进程ID时发生异常: {str(e)}")
        return jsonify({'success': True, 'pid': None})
...
def create_tray_icon():
    # 加载图标
    icon_path = os.path.join(app.config['BASE_DIR'], 'static', 'images', 'qilinssl-logo.ico')
    image = Image.open(icon_path)
    
    def open_browser():
        webbrowser.open('http://127.0.0.1:2002')
    
    def open_app_location():
...
# 检查开机自启动状态
    ...
    # 创建托盘图标菜单
    menu = Menu(
        MenuItem('打开qilin SSL', open_browser),
        MenuItem('查看配置目录', open_app_location),
        MenuItem('退出', quit_app),
        MenuItem('开机自启动', on_clicked_startup, checked=lambda item: is_startup_enabled())
    )
    
    # 创建托盘图标
    icon = Icon('qilin SSL', image, menu=menu)
